#!/bin/bash
# ============================================================
# --compilation-config 批量测试脚本 (v2)
# 测试 mode 和 dynamic_shapes_config 的所有配置组合
# - 添加 warmup 预热
# - 使用 3 条不同长度的问句验证动态 shape
# - 与 NONE baseline 做正确性比对
# - 配置通过环境变量传递（避免 JSON 注入）
# ============================================================
...
LOG_DIR="./test_logs/compilation_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
PYEOF
    ...
    if [ $exit_code -eq 0 ]; then
            ...
            for i in 0 1 2; do
                ...
                if [ -n "$cur" ] && [ -n "$base" ]; then
                    if [ "$cur" = "$base" ]; then
                        ...
                        echo "    OUTPUT_${i}: ⚠️  与 baseline 不一致"
        ...
        local tp=$(grep "^THROUGHPUT=" "$logfile" 2>/dev/null | tail -1 | cut -d= -f2)
...
echo "============================================================"
echo "--compilation-config 批量测试 v2"
echo "============================================================"
echo "模型:       $MODEL"
echo "Python:     $PYTHON"
...
# 保存 baseline（第一个测试完成后的日志，用通配符匹配避免文件名依赖）
...
run_test "mode=VLLM_COMPILE (ACL Graph)" \
    '{"mode": "VLLM_COMPILE"}' \
    ""
...
run_test "VLLM_COMPILE + UNBACKED (自动回退到BACKED)" \
    '{"mode": "VLLM_COMPILE", "dynamic_shapes_config": {"type": "unbacked", "evaluate_guards": false}}' \
    ""

run_test "VLLM_COMPILE + evaluate_guards=True (自动设False)" \
    '{"mode": "VLLM_COMPILE", "dynamic_shapes_config": {"evaluate_guards": true}}' \
    ""

run_test "DYNAMO_TRACE_ONCE + UNBACKED (直接透传)" \
    '{"mode": "DYNAMO_TRACE_ONCE", "dynamic_shapes_config": {"type": "unbacked", "evaluate_guards": false}}' \
    "VLLM_USE_BYTECODE_HOOK=0"

run_test "NONE + BACKED (直接透传)" \
