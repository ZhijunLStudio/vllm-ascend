...
| prefetch_g8n2s1 | 59.4 | 0.07x | ✅ |
| prefetch_g8n2s2 | 66.3 | 0.08x | ✅ |
| prefetch_g4n1s1 | 61.5 | 0.07x | ✅ |
| prefetch_g4n1s2 | 68.3 | 0.08x | ✅ |
...
| Graph 模式兼容 | ✅ 正常 | ✅ 正常 | CUDA Graph + prefetch 已验证通过，与 NPU ACL Graph 一致 |
...
**CUDA 端已完整验证 eager + graph 两种模式**。CUDA Graph + prefetch offload **正常工作**，所有 4 个 offload 配置在 graph 模式下均成功完成 benchmark，无崩溃。此前报告中"CUDA Graph 模式会崩溃"的结论是因为 GPU 显存不足导致 OOM，而非 prefetch 与 CUDA Graph 的代码兼容性问题。
...
两种测试行为不一致。**经实测验证**，CUDA Graph + prefetch **完全兼容**（见第十三节验证清单）。`test_cpu_offload.py` 中的 `--enforce-eager` 仅针对 UVA 后端，prefetch 后端的 graph 兼容性不受此限制。
...
- **NPU ACL Graph 与 prefetch offload 完全兼容**（已验证），**CUDA Graph 与 prefetch offload 也完全兼容**（已实测验证，见第十三节）
...
以下项目已在 CUDA 机器上完成验证（2026-05-09，NVIDIA A800-SXM4-80GB，vLLM 0.16.0rc2）。
...
| 验证项 | 命令 | 预期结果 | 实际结果 |
|--------|------|---------|---------|
| Graph 功能测试 (g8n2s1) | `--mode graph --skip-unit` | 生成正常 | ✅ PASS (59.4 tok/s) |
| Graph 功能测试 (g8n2s2) | `--mode graph --skip-unit` | 生成正常 | ✅ PASS (66.3 tok/s) |
| Graph 功能测试 (g4n1s1) | `--mode graph --skip-unit` | 生成正常 | ✅ PASS (61.5 tok/s) |
| Graph 功能测试 (g4n1s2) | `--mode graph --skip-unit` | 生成正常 | ✅ PASS (68.3 tok/s) |
| Graph 性能基准 | `--mode graph` | 完整 benchmark | ✅ 5/5 配置通过 |
| Graph 输出一致性 | `--mode both` | output_match=True | ✅ 所有配置 output_match=True |
...
| Graph | 867.4 ✅ | 59-68 ✅ | True ✅ | 已验证，无崩溃 |

### 13.5 关键问题
1. **CUDA Graph + prefetch 能不能跑？** ✅ **能跑！** 所有 4 个 offload 配置均成功完成 benchmark，无崩溃。上游代码设计正确，`test_cpu_offload.py` 加 `--enforce-eager` 可能是因为旧版本 vLLM 的兼容性问题，当前版本（v0.16）已修复。
2. **之前的崩溃原因？** GPU 显存不足（其他进程占用），而非代码逻辑问题。在有足够空闲显存的 GPU 上（本次验证使用 80GB A800，空闲 81GB），graph capture 和 replay 均正常。
3. **性能数据：** CUDA Graph baseline 867.4 tok/s，offload 59-68 tok/s（0.07-0.08x）。NPU ACL Graph baseline 475.4 tok/s，offload ~70 tok/s（0.15x）。Graph 模式下 CUDA 和 NPU 的 offload 绝对吞吐接近（59-68 vs 70），说明瓶颈均为 CPU↔设备带宽。
